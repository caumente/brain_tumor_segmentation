...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.val == 0:
            ...
            mode = "val"
            with torch.no_grad():
                validation_loss = step(val_loader, model, mode, criterion, optimizer, epoch,
                                       save_folder=args.save_folder, patients_perf=patients_perf, device=device,
                                       auto_cast_bool=args.auto_cast_bool)
...
def step(
    ...
    for i, batch in enumerate(data_loader):
        ...
        ground_truth = batch["grade"].to(device)
