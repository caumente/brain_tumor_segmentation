...
from src.models.layers import DoubleLevelBlock
...
class DoubleShallowUNet(nn.Module):
    ...
    name = "DoubleShallowUNet U-Net"

    def __init__(self, sequences, regions, width, deep_supervision):
        super(DoubleShallowUNet, self).__init__()

        self.deep_supervision = deep_supervision
        widths = [width * 2 ** i for i in range(4)]

        # Encoders
        self.encoder1 = DoubleLevelBlock(sequences, widths[0] // 2, widths[0])
        self.encoder2 = DoubleLevelBlock(widths[0], widths[1] // 2, widths[1])
        self.encoder3 = DoubleLevelBlock(widths[1], widths[2] // 2, widths[2])
